...
app.delete("/delete-image", async (req,res)=> {
    ...
    try {
        ...
        if(fs.existsSync(filePath)) {
            ...
            res
            .status(200)
            .json({erorr:true, message: error.message})
        }
    } catch(error) {
        res.status(500).json({error:true, message:error.message})    }
})
...
app.post('/add-travel-story', authenticateToken ,async (req,res)=> {
    const { title, story, visitedLocation, isFavourite, createdOn, imageUrl, visitedDate } = req.body
    const { userId } = req.user
...
app.get('/travel-stories/filter', authenticateToken ,async (req,res)=> {
    ...
    try {
        ...
        //Find travel stories that belong to the authetnticated user and fall within the date range
